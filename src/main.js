...
renderFilmCard(FILM_COUNT, filmContainerElement);

// добавление 2-х карточек фильма к Top rated и Most comment.
render(filmsElement, createTopRatedTemplate(), `beforeend`);
const topRatedElement = filmsElement.querySelector(`.films-list--extra`);
const extraTopRatedElement = topRatedElement.querySelector(`.films-list__container`);
renderFilmCard(FILM_EXTRA_COUNT, extraTopRatedElement);

render(filmsElement, createMostCommentedTemplate(), `beforeend`);
const mostCommentElement = filmsElement.children[2];
const extraMostCommentElement = mostCommentElement.querySelector(`.films-list__container`);
renderFilmCard(FILM_EXTRA_COUNT, extraMostCommentElement);

/* или так еще..
const filmExtraListElement = filmsElement.children;
render(filmExtraListElement[1].children[1], createFilmCardTemplate(), `beforeend`);
render(filmExtraListElement[1].children[1], createFilmCardTemplate(), `beforeend`);
render(filmExtraListElement[2].children[1], createFilmCardTemplate(), `beforeend`);
render(filmExtraListElement[2].children[1], createFilmCardTemplate(), `beforeend`);
*/
